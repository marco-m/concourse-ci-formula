...
# Disable escaping from any shared directory via symbolic links, new VirtualBox
...
Vagrant.configure("2") do |config|
  ...
  # a problem; it becomes a problem because we want to use the same configuration
  ...
  config.vm.provision "shell", inline: <<-EOF
    # Fix DNS resolution
    sed --in-place \
    's/hosts:.*/hosts: files mymachines myhostname dns resolve/' /etc/nsswitch.conf
    ...
    # Since we updated the system, it is possible for the kernel to be out of sync
    # with iptables, which is used by concourse worker. We need to reboot.
    #echo rebooting
    #reboot
  EOF
